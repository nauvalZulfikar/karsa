...
(async () => {
  ...
  try {
    ...
    await page.screenshot({ path: `${OUT}/01_open.png`, fullPage: false });
    log('type message...');
    await page.locator('input.ai-fc-input').fill('halo bos');
    await page.locator('button.ai-fc-send').click();
    await page.waitForTimeout(8000); // wait for reply
    await page.screenshot({ path: `${OUT}/02_replied.png`, fullPage: false });
