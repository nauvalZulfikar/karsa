...
const OUT = resolve(import.meta.dirname, '..', '..', 'tmp', 'smoke', 'modal_dark');
...
(async () => {
  ...
  await page.screenshot({ path: `${OUT}/01_dashboard_dark.png` });
  ...
  if (await card.count() > 0) {
    ...
    await page.screenshot({ path: `${OUT}/02_info_tab_dark.png` });
    ...
    await page.screenshot({ path: `${OUT}/03_milestone_dark.png` });

    // Scroll down
    const modal = page.locator('.kanban-modal');
    await modal.evaluate(el => el.scrollTop = 300);
    await page.waitForTimeout(300);
    await page.screenshot({ path: `${OUT}/04_milestone_scrolled_dark.png` });
  }
